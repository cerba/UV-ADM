...
# Last Update 11/5/19 adds new good_up condition:
...
##for i, x, in enumerate( np.array([0.6])): # one ray; pick a single x; np.array([0]) is x=0 plane
    
    for j, y in enumerate(ylist): #all rays
    ##for j, y, in enumerate( np.array([0])): # one ray; pick a single y
	...
	## Step 7: Calculate Absorption, Emission
        if x**2+y**2 <=1.0:
